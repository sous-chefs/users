...
describe user('test_user_custom_paths') do
  it { should exist }
  its('uid') { should eq 9003 }
  its('groups') { should eq %w( test_user_custom_paths testgroup nfsgroup ) }
  its('shell') { should eq '/bin/bash' }
end

custom_paths_ssh_keys = [
  'ssh-rsa  chefuser@mylaptop.local',
]

describe file('/etc/ssh/ssh.keys/test_user_custom_paths.pub') do
  custom_paths_ssh_keys.each do |key|
    its('content') { should include(key) }
  end
end
