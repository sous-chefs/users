...
describe 'users::sysadmins' do
  ...
  context 'Resource "users_manage"' do
    ...
    it 'creates user "createme"' do
      expect(chef_run).to create_user('createme').with(uid: 1234,
                                                       gid: 4321,
                                                       home: '/home/createme')
    ...
    it 'creates user "createdevnull"' do
      expect(chef_run).to create_user('createdevnull').with(home: '/dev/null')
      expect(chef_run).to_not create_directory('/dev/null/.ssh')
    end

    it 'creates group "createme"' do
      expect(chef_run).to create_group('createme').with(gid: 4321)
    end

    it 'creates group "sysadmin"' do
      expect(chef_run).to create_group('sysadmin').with(gid: 2300)
    end
